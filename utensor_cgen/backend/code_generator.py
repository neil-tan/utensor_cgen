...
class CodeGenerator(object):
  ...
  def _generate(self, src_fname, ugraph):
    ...
    # NT: Generating Tensor Naming file; expecting no changes to the graph from this point on
    tensor_name_index_list = self._get_tensor_name_index_list(quant_ugraph)
    tensor_name_snippet = TensorNamesHeaderSnippet(guard_name, tensor_name_index_list)
    tensor_name_header_fname = '{}_tensor_names.hpp'.format(fname)
    _logger.info("Generate tensor name file: %s", tensor_name_header_fname)
    with open(tensor_name_header_fname, "w") as wf:
      wf.write('// Auto generated by utensor-cli\n\n')
      wf.write(tensor_name_snippet.render())
  ...
  def _get_tensor_name_index_list(self, ugraph):
    ...
    for _, op_name in enumerate(ugraph.topo_order):
      ...
      names = [it_tensor.name for it_tensor in op_info.input_tensors]
